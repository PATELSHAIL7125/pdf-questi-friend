...
const PDFViewer: React.FC = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center px-4 animate-fade-in">
      <div className="relative w-full max-w-3xl mb-4 glass-panel rounded-xl p-4 overflow-hidden">
        {isLoading && (
      ...
      <div className="flex items-center justify-center w-full max-w-md mb-2">
        <Button 
          ...
          size="sm" 
          onClick={zoomOut}
          className="button-transition mr-2"
        >
          <ZoomOut className="h-4 w-4" />
        </Button>
        <span className="text-sm mx-2">Zoom: {Math.round(scale * 100)}%</span>
        <Button 
          variant="outline" 
          size="sm" 
          onClick={zoomIn}
          className="button-transition ml-2"
        >
